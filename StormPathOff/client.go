...
// encountered.
func (client *Client) Request(method string, url string, body interface{}) (*http.Response, error) {
	jsonBody, _ := json.Marshal(body)
	if strings.HasPrefix(url, "/") {
		url = BASE_URL + url
	}
	//	fmt.Printf("Mehtod %s URL %s\n", method, url)
	req, _ := http.NewRequest(method, url, bytes.NewReader(jsonBody))
	req.Header.Set("User-Agent", USER_AGENT)
	req.Header.Set("Accept", "application/json")
	req.Header.Set("Content-Type", "application/json")
	//	req.Header.Set("Stormpath-Go-FollowRedirects", "false")
	...
	//	fmt.Printf("Request for Sending ... %v\n", httpClient)
	if err != nil {
		fmt.Println("ERROR in Client.go")
		return nil, err
	}
	return resp, nil
}

func (client *Client) VDo(method string, url string, body interface{}) (*http.Response, error) {
	jsonBody, _ := json.Marshal(body)
	if strings.HasPrefix(url, "/") {
		url = BASE_URL + url
	}
...
func (client *Client) LRSCall(method string, url string, body interface{}) (*http.Response, error) {
	jsonBody, _ := json.Marshal(body)
	if strings.HasPrefix(url, "/") {
		url = BASE_URL + url
	}
